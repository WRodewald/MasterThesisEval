...
FinishPage.prototype.render = function (_parent) {
    ...
    for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
      ...
      if (element.type === "text") {
        ...
        table.append($("<tr><td><strong>"+ element.label + optchar +"</strong></td><td><input id='"+element.name+"' /></td></tr>"));
      } else if (element.type === "number") {
        if(element.default != undefined) {
          table.append($("<tr><td><strong>"+ element.label + optchar +"</strong></td><td><input id='"+element.name+"' min='"+element.min+"' max='"+element.max+"' value='"+element.default+"' data-inline='true'/></td></tr>"));
        } else {
          table.append($("<tr><td><strong>"+ element.label + optchar +"</strong></td><td><input id='"+element.name+"' min='"+element.min+"' max='"+element.max+"' data-inline='true'/></td></tr>"));
        } 
        ...
        table.append($("<tr></tr>").append(
          $("<td><strong>"+ element.label + optchar +"</strong></td>"),
          td
        ));
        this.likert.render(td);        
      }else if (element.type === "long_text"){
        
        table.append($("<tr><td id='labeltd' style='vertical-align:top; padding-top:"+ $('#feedback').css('margin-top') +"'><strong>"+ element.label + optchar +"</strong></td><td><textarea name='"+element.name+"' id='"+element.name+"'></textarea></td></tr>"));      
      }
      console.log(element);    
    }
    ...
    if (this.pageConfig.showResults) {
      ...
      for (i = 0; i < trials.length; ++i) {
        ...
        if (trial.type === "mushra") {
	     ...
	     } else if (trial.type === "quality") {
        trT = document.createElement("tr");        
        thT = $("<th colspan='2'></th>");
        $(thT).append(trial.id + " (QUALITY)" );
        $(trT).append(thT);
        $(table).append(trT);
