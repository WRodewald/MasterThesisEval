...
function FilePlayer(_audioContext, _bufferSize, _stimuli, _errorHandler, _language, _localizer, _description = []) {
...
FilePlayer.prototype.renderElement = function(_parent, _i) {
  var buttonPlay = $('<button data-role="button" id="playCondition'+_i+'" name="'+_i+'">' + this.localizer.getFragment(this.language, 'playButton')  + '</button>');
    ...
    var label = $('');
    if(this.description.length == this.stimuli.length) {
      label = $('<td></td>').append("<b style='padding-right:2em;'>" + this.description[_i] + "</b>")
    }
...
FilePlayer.prototype.render = function(_parent) {
  ...
  var table = $('<table align="center" class=""></table>');
