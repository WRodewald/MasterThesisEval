...
function MushraAudioControl(_audioContext, _bufferSize, _reference, _conditions, _errorHandler, _createAnchor35, _createAnchor70, _randomize, _flip_order = false) {
  ...
  this.audioFadingMaxPosition = parseInt(audioContext.sampleRate * 0.01);     // 0.005
  this.audioMinimumLoopDuration = parseInt(audioContext.sampleRate * 0.5);     // 0.5
  this.audioVolume = 1.0;
  this.audioIsReferencePlaying = null;
  
  // requests
  this.audioCurrentPositionRequest = null; 
  this.audioCurrentPositionRequestFading = null; 
...
MushraAudioControl.prototype.process = function(audioProcessingEvent) {
  ...
  var audioCurrentPositionFading = this.audioCurrentPositionRequestFading;
      ...
      if (fadingActive == 1) { // fade out
        ...
        if (fadingCurrentPosition >= this.audioFadingMaxPosition) {          
          ...
          if (this.audioFadingIn === null) {
            ...
            if(audioCurrentPositionFading != null)
            {
              currentPosition = audioCurrentPositionFading;
              this.audioCurrentPositionRequestFading = null;
            }
          }
...
MushraAudioControl.prototype.setPositionFading = function(_position) {
  ...
  if(_position < this.audioLoopStart)
  {
    this.audioCurrentPositionRequestFading = this.audioLoopStart;
    this.fadeOut(this.audioStimulus)
  }
  else if(_position < this.audioLoopEnd)
  {
    this.audioCurrentPositionRequestFading = _position;
    this.fadeOut(this.audioStimulus)
  }
  else
  {
